    ...
    botMessage.awaitReactions({ filter, max: 1, time: 300000, errors: ["time"] }).then(async collected => {
      if (collected.first().emoji.name === "✅") {
        con.query(`SELECT * FROM PlayerElo WHERE name = '${username}'`, function(error, results) {
          ...
          con.query(`UPDATE PlayerElo SET ${remainingContent[2].toLowerCase().toLowerCase()} = '${calculateElo(gamemodeElo, remainingContent[3])}' WHERE name = '${username}';`, function(err) {
            if (err) throw err;
          });
          client.channels.cache.get(config.channels.botLogsChannel).send(`${message.author.tag} removed ${remainingContent[3]} from ${username}'s ${remainingContent[2].toLowerCase()} elo`);
          botMessage.reply({ embeds: [new Discord.MessageEmbed().setTitle("Done!").setColor("#4BB543").setDescription(`Successfully removed **${remainingContent[3]}** elo from **${username}**'s **${remainingContent[2].toLowerCase()}** statistics. They now have a total of **${calculateElo(gamemodeElo, remainingContent[3])}** elo in **${remainingContent[2].toLowerCase()}**.`).setFooter(`Command requested by ${message.author.tag}`, message.author.displayAvatarURL())] });
        });
...
exports.help = {
  ...
  description: "Removes elo from a player",
  usage: "removeelo \"[username]\" [gamemode] [amount]"
};