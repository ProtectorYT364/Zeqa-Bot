...
exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  ...
  if (args[0] == null) return message.reply(`<:cross:883326239341965332> | Usage: ${settings.prefix}removeadmin [username#0000]`);
  ...
  client.guilds.fetch(config.Guild).then(guild => {
    ...
    extraConnection.query(`SELECT * FROM SMPInviterData WHERE discord = '${discordID}'`, (err, result) => {
      ...
      extraConnection.query(`UPDATE SMPInviterData SET isadmin = 0 WHERE discord = ${discordID}`, function(err) {
        if (err) throw err;

        message.reply({ embeds: [new Discord.MessageEmbed().setTitle("Done!").setColor("#4BB543").setDescription(`Removed administrator permissions from ${discordUsername} on the SMP!`).setFooter(`Command requested by ${message.author.tag}`, message.author.displayAvatarURL())] });
      });
    });
  });
};
...
exports.help = {
  name: "removeadmin",
  category: "SMP",
  description: "Removes administrator permissons from a player on the SMP",
  usage: "removeadmin [username#0000]"
};